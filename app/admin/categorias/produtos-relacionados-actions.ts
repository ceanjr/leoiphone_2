...
import { gerarDescontoAleatorio } from '@/lib/utils/desconto-colors'

/**
 * Busca a configuração global de produtos relacionados
 */
export async function getConfigGlobalProdutosRelacionados(): Promise<{
  data: { id: string; ativo: boolean; desconto_min: number; desconto_max: number } | null
  error: string | null
}> {
  try {
...
export async function getProdutosRelacionados(
  ...
  try {
    ...
    console.log('[getProdutosRelacionados] Config categoria:', config)

    if (!config) {
      console.log('[getProdutosRelacionados] Sem config de categoria, retornando vazio')
      return { data: [], error: null }
    }
      ...
      if (todasCategorias && categoriaAtual) {
        ...
        if (categoriasIds.length > 0) {
          ...
          if (produtosAuto && produtosAuto.length > 0) {
            // Aleatorizar e pegar apenas o necessário
            const shuffled = produtosAuto.sort(() => 0.5 - Math.random())
        ...
        if (produtosRelacionados.length < limit) {
          ...
          if (produtosFallback && produtosFallback.length > 0) {
            const shuffled = produtosFallback.sort(() => 0.5 - Math.random())
    ...
    // Aplicar desconto aleatório para cada produto
    const produtosComDesconto = produtosRelacionados.slice(0, limit).map((produto) => {
      const descontoAleatorio = gerarDescontoAleatorio(config.desconto_min, config.desconto_max)
      return {
        ...produto,
        preco_original: produto.preco,
        preco_com_desconto: produto.preco * (1 - descontoAleatorio / 100),
        desconto_percentual: descontoAleatorio,
      }
    })
